...
|---|---|---|
| `S` Spoofing: podszycie się pod inny węzeł przez fałszywy `src_id` | HMAC-SHA256 dla `LAVIET_FRAME_V1`, lokalny `node_id`, pairing/trust lista, root seed w TPM NV | Dodać policy dla operacji administracyjnych i pełny flow admin/service |
...
| `T` Tampering: manipulacja stanem EEPROM / listą trusted | Częściowe formaty z CRC i wersjonowaniem danych, klucz EEPROM wyprowadzany z TPM-backed root seeda | Dodać integralność rekordów security, licznik wersji i odporność na rollback |
...
| `I` Information Disclosure: podsłuch payloadu `OPERATOR` | `coding` dla secure frames | Domyślnie wymusić secure mode dla ruchu operatora, rotować klucz sieciowy, użyć mocniejszej prymitywy niż obecny software cipher |
| `I` Information Disclosure: wyciek klucza z EEPROM/firmware | Root seed jest trzymany w TPM NV `0x01C10101`, zapis wymaga TPM PP, EEPROM key jest wyprowadzany z root seeda | Ograniczyć obecność klucza w RAM, wyczyścić bufory po użyciu, rozważyć sealing/policy sessions |
...
| `E` Elevation of Privilege: zdalne wykonanie operacji administracyjnej | Część operacji wymaga wejścia do odpowiedniego menu/flow; lokalna rotacja root seeda wymaga TPM PP | Rozdzielić role: operator/admin/service, chronić więcej operacji security przez TPM policy/local presence i osobne komendy admin |
| `E` Elevation of Privilege: przejęcie pairingu bez fizycznej obecności | Pairing nadal wymaga lokalnego flow UI, ale TPM PP chroni obecnie tylko zapis root seeda | Wdrożyć TPM `PolicyPhysicalPresence`, policy session i ograniczenie pairingu do lokalnie autoryzowanego okna |
...
|---|---|---|---|
| `C` Confidentiality | Podsłuch radiowy `OPERATOR` | `coding` dla secure ramek | Wymusić secure-by-default, mocniejsza kryptografia, regularna rotacja klucza sieciowego |
| `C` Confidentiality | Odczyt sekretów z pamięci trwałej | Root seed w TPM NV, EEPROM key z TPM-backed root seeda, migracja legacy EEPROM key | Sealed storage z TPM, policy sessions, ograniczenie debug/logów sekretów |
...
| `I` Integrity | Fałszywe trusted device / nieautoryzowane parowanie | Pairing flow i trusted list | Pairing challenge-response, TPM RNG, lokalna zgoda fizyczna przez TPM policy |
...
3. Rozszerzyć ochronę TPM PP/policy z root seeda na pairing i operacje administracyjne.
4. Dalej ograniczać ekspozycję sekretów w RAM i logach UART.
